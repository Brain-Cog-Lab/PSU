...
# parser.add_argument('--wandb', type=bool, default=True, help='use wandb to log training')
parser.add_argument('--dataset', default='shd', type=str) # sew_resnet19
parser.add_argument('--model', default='SHD_SNN', type=str, metavar='MODEL', help='Name of model to train (default: "countception"')
...
parser.add_argument('--eval_checkpoint', default='', type=str, metavar='PATH', help='path to eval checkpoint (default: none)')
...
parser.add_argument('-nc', '--num-classes', type=int, default=20, metavar='N', help='number of label classes (default: 1000)')
...
parser.add_argument('--crop-pct', default=None, type=float, metavar='N', help='inputs image center crop percent (for validation only)')
...
parser.add_argument('-b', '--batch-size', type=int, default=256, metavar='N', help='inputs batch size for training (default: 128)')
...
parser.add_argument('--opt-eps', default=1e-8, type=float, metavar='EPSILON', help='Optimizer Epsilon (default: None, use opt default)')
...
parser.add_argument('--weight-decay', type=float, default=0., help='weight decay (default: 0.01 for adamw)')
...
parser.add_argument('--warmup-lr', type=float, default=1e-6, metavar='LR', help='warmup learning rate (default: 0.0001)')
parser.add_argument('--min-lr', type=float, default=1e-5, metavar='LR', help='lower lr bound for cyclic schedulers that hit 0 (1e-5)')
parser.add_argument('--epochs', type=int, default=120, metavar='N', help='number of epochs to train (default: 2)')
...
parser.add_argument('--remode', type=str, default='pixel', help='Random erase mode (default: "const")')
parser.add_argument('--recount', type=int, default=1, help='Random erase count (default: 1)')
parser.add_argument('--resplit', action='store_true', default=False, help='Do not random erase first (clean) augmentation split')
parser.add_argument('--mixup', type=float, default=0., help='mixup alpha, mixup enabled if > 0. (default: 0.)')
...
parser.add_argument('--mixup-prob', type=float, default=0., help='Probability of performing mixup or cutmix when either/both is enabled')
...
parser.add_argument('--newton-maxiter', default=20, type=int, help='max iterration in newton method')
parser.add_argument('--reset-drop', action='store_true', default=False, help='whether to reset drop')
parser.add_argument('--kernel-method', type=str, default='cuda', choices=['torch', 'cuda'], help='The implementation way of gaussian kernel method, choose from "cuda" and "torch"')
...
parser.add_argument('--dist-bn', type=str, default='', help='Distribute BatchNorm stats between node after each epoch ("broadcast", "reduce", or "")')
...
parser.add_argument('--model-ema-decay', type=float, default=0.99996, help='decay factor for model weights moving average (default: 0.9998)')
...
parser.add_argument('--save-images', action='store_true', default=False, help='save images of inputs bathes every log interval for debugging')
...
parser.add_argument('--output', default='/data/ly/BrainCog', type=str, metavar='PATH', help='path to output folder (default: none, current dir)')
...
# Spike parameters
parser.add_argument('-T', '--step', type=int, default=15, help='Simulation time step (default: 10)')
parser.add_argument('--encode', type=str, default='direct', help='Input encode method (default: direct)')
parser.add_argument('--temporal-flatten', action='store_true', help='Temporal flatten to channels. ONLY FOR EVENT DATA TRAINING BY ANN')
...
# neuron type
parser.add_argument('-n', '--node-type', type=str, default='PLIFNode', help='Node type in network (default: PLIF)')
parser.add_argument('--act-fun', type=str, default='AtanGrad', help='Surogate Function in node. Only for Surrogate nodes (default: AtanGrad)')
parser.add_argument('--threshold', type=float, default=.3, help='Firing threshold (default: 0.5)')
...
parser.add_argument('--loss-fn', type=str, default='mse', help='loss function (default: ce)')
parser.add_argument('--noisy-grad', type=float, default=0., help='Add noise to backward, sometime will make higher accuracy (default: 0.)')
parser.add_argument('--spike-output', action='store_true', default=False, help='Using mem output or spike output (default: False)')
...
parser.add_argument('-lbl', '--layer-by-layer', action='store_true', help='forward step-by-step or layer-by-layer. Larger Model with layer-by-layer will be faster (default: False)')
# parser.add_argument('-lbl', '--layer-by-layer', default=True, type=bool, help='forward step-by-step or layer-by-layer. Larger Model with layer-by-layer will be faster (default: False)')
parser.add_argument('-tet', '--tet-loss', action='store_true')
...
parser.add_argument('--cutmix_noise', type=float, default=0., help='Add Pepper noise after mix, sometimes work (default: 0.)')
parser.add_argument('--gaussian-n', type=int, default=3)
parser.add_argument('--rand-aug', action='store_true', help='Rand Augment for Event data (default: False)')
parser.add_argument('--randaug_n', type=int, default=3, help='Rand Augment times n (default: 3)')
parser.add_argument('--randaug_m', type=int, default=15, help='Rand Augment times n (default: 15) (0-30)')
parser.add_argument('--train-portion', type=float, default=0.9, help='Dataset portion, only for datasets which do not have validation set (default: 0.9)')
parser.add_argument('--event-size', default=48, type=int, help='Event size. Resize event data before process (default: 48)')
parser.add_argument('--node-resume', type=str, default='', help='resume weights in node for adaptive node. (default: False)')

# visualize
parser.add_argument('--visualize', action='store_true', help='Visualize spiking map for each layer, only for validate (default: False)')
parser.add_argument('--spike-rate', action='store_true', help='Print spiking rate for each layer, only for validate(default: False)')
...
parser.add_argument('--suffix', type=str, default='', help='Add an additional suffix to the save path (default: \'\')')
...
global train_loss, test_loss, train_acc, test_acc
...
def main():
    ...
    if args.node_type in ['SPSN', 'PSU', 'IPSU', 'RPSU']:
        args.layer_by_layer = True
        print('parallel node with layer-by-layer')
    ...
    if args.wandb:
        ...
        wandb.init(project='DPLIF', config=vars(args), name=wandb_name)
...
def train_epoch(
    ...
    return OrderedDict([('loss', losses_m.avg), ('time', np.sum(time_train))])
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, target) in enumerate(loader):
            ...
            with amp_autocast():
                start = time.time()
                output = model(inputs)
                time_eval.append(time.time() - start)
        ...
        test_loss = losses_m.avg
        test_acc = top1_m.avg

    # metrics = OrderedDict([('loss', losses_m.avg), ('top1', top1_m.avg), ('top5', top5_m.avg)])
    metrics = OrderedDict([('loss', losses_m.avg), ('top1', top1_m.avg), ('time', np.sum(time_eval))])
