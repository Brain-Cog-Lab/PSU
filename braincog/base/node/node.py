...
class LIFNode(BaseNode):
    ...
    def integral(self, inputs):
        ...
        # self.mem = self.mem * (1 - 1./self.tau) + inputs

    def calc_spike(self):
        self.spike = self.act_fun(self.mem - self.threshold)
        # self.mem = self.mem * (1 - self.spike.detach())
        self.mem = self.mem - self.spike.detach() * self.threshold
        self.sum_spike += self.spike.detach()
...
class PSN(BaseNode):
    ...
    def integral(self, inputs):
        ...
        # self.mem = torch.mm(torch.sigmoid(torch.tril(self.w1)), inputs.flatten(1)).view(inputs.shape)
        self.mem = self.fc(inputs.flatten(1).permute(1, 0)).permute(1, 0)
        # self.mem = self.mask_weight()(inputs.flatten(1))
    def calc_spike(self):
        self.spike = self.act_fun(self.mem).view(self.shape)


class MaskedPSN(BaseNode):
    ...
    def integral(self, inputs):
        self.shape = inputs.shape
        self.mem = torch.addmm(self.fc.bias.unsqueeze(1), self.mask_weight(), inputs.flatten((1)))
...
class PLIFNode1(BaseNode):
    def __init__(self, threshold=1., tau=2., act_fun=AtanGrad, *args, **kwargs):
        super().__init__(threshold, *args, **kwargs)
        init_w = -math.log((tau - 1.)/2)
        if isinstance(act_fun, str):
            act_fun = eval(act_fun)
        self.act_fun = act_fun(alpha=2., requires_grad=True)
        self.w = nn.Parameter(torch.as_tensor(init_w))
    ...
    def integral(self, inputs):
        self.mem = self.mem + ((inputs - self.mem) * (self.w * (self.t+1) * 2).sigmoid()) * self.dt
        self.t += 1
...
class DPLIFNode(BaseNode):
    def __init__(self, threshold=1., tau=2., act_fun=AtanGrad, *args, **kwargs):
        ...
        T = self.step
        # self.w1 = nn.Parameter(torch.as_tensor(init_w))
        # self.w2 = nn.Parameter(torch.as_tensor(init_w))

        self.w1 = nn.Parameter(torch.as_tensor(init_w))
        self.w2 = nn.Parameter(torch.as_tensor(init_w))

    def integral(self, inputs):
        # self.mem = self.mem + ((inputs - self.mem) * self.w1[self.t].sigmoid())
        # self.mem = self.mem * self.w1[self.t].sigmoid() + inputs * self.w2[self.t].sigmoid()
        # self.mem = self.mem * self.w1.sigmoid() + inputs
        self.mem = self.mem * self.w1.sigmoid() + inputs * self.w2.sigmoid()
        # self.mem = self.mem * self.w1 + inputs * self.w2
        self.t += 1
...
class DPLIFNode1(BaseNode):
    def __init__(self, threshold=1., tau=2., act_fun=AtanGrad, *args, **kwargs):
        ...
        if isinstance(act_fun, str):
            act_fun = eval(act_fun)
        self.act_fun = act_fun(alpha=2., requires_grad=True)
        self.w = nn.Parameter(torch.as_tensor(init_w * torch.ones([self.step])))

    def integral(self, inputs):
        self.mem = self.mem + ((inputs - self.mem) * self.w[self.t].sigmoid()) * self.dt
        self.t += 1
...
class DPLIFNode2(BaseNode):
    ...
    def integral(self, inputs):
        self.mem = self.mem + ((inputs - self.mem) * (self.w[self.t] * (self.t+1) * 2).sigmoid()) * self.dt
        self.t += 1
...
def generate_matrix(T, tau):
    matrix = torch.tril(torch.ones([T, T]) / tau)
    matrix2 = torch.tril(torch.ones([T, T]) / tau, diagonal=-1)

    for i in range(T):
        for j in range(T):
            if i > j:
                matrix[i][j] = (1 / tau) ** (i - j + 1)

    for i in range(T):
        for j in range(T):
            if i > j:
                matrix2[i][j] = - (1 / tau) ** (i - j)
